  ...
  actions: {
    ...
    async queryBlocksInRange(
      ...
      try {
        ...
        const minHeight = offset ? latestHeight - 9 * offset : latestHeight - 9; //non-offset
        const maxHeight = offset
          ? latestHeight - 9 * (offset - 1)
          : latestHeight; //non-offset

        console.log({ minHeight, maxHeight });

        const blocks = await axios.get(
          `${rootGetters["common/env/apiTendermint"]}/blockchain?minHeight=${minHeight}&maxHeight=${maxHeight}`
        );

        commit("QUERY", {
          query: "initialBlocks",
          key: { params: { ...key }, query },
          value: { blocks: blocks.data.result.block_metas, latestHeight },
        });
